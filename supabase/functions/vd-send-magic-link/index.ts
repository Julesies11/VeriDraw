// supabase/functions/vd-send-magic-link/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Create a service role client to generate the magic link
    ...
    // 2. Generate the login link securely
    ...
    if (!resendApiKey) {
      console.warn("RESEND_API_KEY is not set. Action link logged to console:");
      console.log(`[TESTING] Magic link for ${email}: ${actionLink}`);
      
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: "Magic link generated but RESEND_API_KEY is not configured. Link logged to Deno console logs for testing." 
        }),
        ...
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 500px; margin: 40px auto; padding: 32px; border: 1px solid #e5e7eb; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
            ...
            <p style="font-size: 14px; color: #4b5563; line-height: 1.6; margin-bottom: 24px; text-align: center;">
              Click the button below to securely sign in to your VeriDraw dashboard. This link will expire in 1 hour.
            </p>
    ...
    console.error(`[vd-send-magic-link Error]`, error);
    return new Response(
