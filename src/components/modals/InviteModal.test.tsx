...
describe('InviteModal', () => {
  it('renders nothing when isOpen is false', () => {
    const { container } = render(
      <InviteModal isOpen={false} onClose={vi.fn()} inviteCode="VD-ABC123" eventSlug="vd-abc123" />
    );
  ...
  it('renders the modal heading when isOpen is true', () => {
    const { getByText } = render(
      <InviteModal isOpen={true} onClose={vi.fn()} inviteCode="VD-ABC123" eventSlug="vd-abc123" />
    );
  ...
  it('calls onClose when the close (✕) button is clicked', () => {
    const onClose = vi.fn();
    const { container } = render(
      <InviteModal isOpen={true} onClose={onClose} inviteCode="VD-ABC123" eventSlug="vd-abc123" />
    );
  ...
  it('copies the direct /draw/ link to clipboard when the copy-link button is clicked', () => {
    const { container } = render(
      <InviteModal isOpen={true} onClose={vi.fn()} inviteCode="VD-ABC123" eventSlug="vd-abc123" />
    );
  ...
  it('displays the correct /draw/ URL in the read-only direct-link input', () => {
    const { container } = render(
      <InviteModal isOpen={true} onClose={vi.fn()} inviteCode="VD-XYZ789" eventSlug="vd-xyz789" />
    );
